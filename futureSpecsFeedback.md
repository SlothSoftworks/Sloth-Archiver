...
engineering one. Its prerequisite — multi-version tracking (this file's own
"Download new version"/hotswap item) — is now built *(2026-08-03)*, so this no longer
needs to wait on anything, but the rating doesn't change: the hard part was always the
comparison mechanism itself, not the versioning scaffolding around it.

Breaking down the actual comparison mechanism, now that versioning exists:
...
| Per-video metadata persistence (`<base>/<channel>/<videoId>/<epoch>/metadata.json`) | **Medium-High** — ✅ done | Straightforward to write, but channel names and video titles are attacker-adjacent, uncontrolled strings — needed real sanitization for illegal filesystem characters, unicode, and Windows' reserved device names (found and fixed a real bug here — bare `channelFolderName('CON')` was unsafe) and MAX_PATH risk (logged as **TD-005**). Video folders were originally named `<title> [<videoId>]`, but titles can drift after upload (YouTube allows edits) causing cosmetic staleness with no live functional bug, since nothing re-derives paths from title; switched *(2026-08-03)* to bare `videoId` naming, incidentally shrinking TD-005's worst case too since videoIds are far shorter and fixed-length. No migration needed — `scanLibrary()` identifies videos by folder contents, never the folder name. |
...
| Download-status/quality badge on cards | **Low** — ✅ done | Pure rendering off persisted metadata (`downloadedResolution`/`downloadedFormat`), shown on both the mini grid cards and the full detail view. Updated *(2026-08-03)* once versions landed: the grid-view badge now reflects the best quality downloaded across *all* versions of a video (preferring a real video resolution over an MP3-only capture), not just the latest one, plus a "{n} versions" caption when there's more than one — scoped to the grid view only, since the detail view already exposes this via its version selector. |
...
| "Download new version" (new epoch folder + version hotswap) | **Very High** — ✅ done *(2026-08-03)* | Built as an `epochs[]` array per video (newest first) alongside the existing `latestEpoch`/`metadata` fields, via a new `addLibraryVersion()`/`buildEpochMetadata()` pair in `library.mjs` and a `library:addVersion` IPC. The "Add as new version" duplicate-dialog button (previously permanently disabled) is now wired up, and the detail view gained a version selector (`Select`, only shown when there's more than one epoch) with a small `DownloadDoneIcon` per entry marking which versions have a file downloaded locally. `deleteLibraryEntry` was made version-aware (deletes one epoch, removes the whole video folder only once none remain). Turned out to be exactly as architecturally open-ended as rated: three separate staleness bugs surfaced in testing, all the same root cause — child components rendering off parent-held `selectedVideo`/`selectedChannel` snapshots that weren't being re-synced after a mutation — fixed by generalizing the refresh callback (`onVersionsChanged`) to do a deep re-sync instead of just refreshing the shallow channel list, plus a forced remount (`key={selectedEpoch}`) so the player swaps correctly between local-file and YouTube-embed display on version switch. Logged as **TD-007**: every version mutation currently triggers a full `scanLibrary()` re-walk rather than a localized update. |
...
| Rudimentary version control | **Very High** — ✅ done *(2026-08-03)* | No longer deferred — this and "Download new version" above turned out to be the same feature, not two separate ones, and landed together. See the row above for what was built. |
...
**Still remaining:** channel-vs-video view toggle (still deferred, as agreed) and the
ffmpeg-utilities item, which doesn't depend on anything else in this list and could be
slotted in whenever convenient. Safe quality-swap and new-version/hotswap (plus the
version-control generalization it implied) are both done now, along with two related
follow-ups to already-"done" rows above (the quality badge now reflects the best version,
not just the latest; video folders are now named by videoId instead of title). One small
gap inside an already-"done" row, not yet closed: auto-navigate
to the newly added video after a successful "add to library" (see the cross-tab wiring row
above) — the notification badge remains an interim substitute, not a replacement.
...
| 9. Video merger (re-uploaded/re-edited video → swap primary link, keep the old one as a legacy version) | **Medium, now that its prerequisite is built** | Was gated on multi-version tracking existing first ("Download new version"/hotswap above); that landed *(2026-08-03)*, so the version-control schema this item needed is no longer a blocker. The remaining incremental delta is genuinely small — a "link a replacement URL" dialog that fetches the new video and adds it as a new epoch under the existing entry, plus a status label distinguishing "current" from "legacy" — not yet built, but no longer bottlenecked on anything else in this file. |
